...
const createFormState = ({price, email, confirmationEmail}, hasAgreedTOS, isNewPurchase) => {
    ...
    if (isNewPurchase) {
        if (confirmationEmail.length === 0 || email.length === 0) {
            ...
            return _createFormState(true, "Emails are not the same.")
        }
    }

    if (!hasAgreedTOS) {
        return _createFormState(true, "You must agree with the terms of service.")
    }
...
export default function OrderModal({ course, onClose, onSubmit, isNewPurchase }) {
    ...
    const [enabledPrice, setEnablePrice] = useState(false)
                ...
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div className="sm:flex sm:items-start">
                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            {
                                formState.message && <div className="p-4 my-3 text-yellow-700 bg-yellow-200 rounded-lg text-sm">
                                    {formState.message}
                                </div>
                            }
                        </div>
